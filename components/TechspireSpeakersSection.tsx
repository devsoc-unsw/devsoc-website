...
import { CardType, TechspireCard } from "./ProjectCard";
import PageSection from "./PageSection";
import { AspectRatio, Link, Stack, Typography } from "@mui/material";
import Image from "next/image";
import { techspireHeaderData, techspireSpeakerData } from "../data/techspire";
...
function renderLogoRows(
  ...
  const isPearler =
    sponsor.name && sponsor.name.toLowerCase().includes("pearler");
  const isJobsboard =
    sponsor.name && sponsor.name.toLowerCase().includes("jobsboard");
    ...
    <PageSection title={`${selectedYear}`}>
      ...
      <Stack
        ...
        gridTemplateColumns={{
          sm: "repeat(2, 1fr)",
          md: `repeat(${Math.min(
            techspireHeaderData[selectedYear].supporterLogos.length,
            3
          )}, 1fr)`,
        }}
        marginBottom={8}
        sx={{ gridGap: "64px" }}
      >
        {techspireHeaderData[selectedYear].supporterLogos.map(
          (sponsor, idx) => {
      ...
      {techspireHeaderData[selectedYear].speakerLogos &&
        techspireHeaderData[selectedYear].speakerLogos.length > 0 && (
          <>
            <Typography level="h3" py={2} textAlign={"center"}>
              Speakers from
            </Typography>
            <Stack
              ...
              sx={{ gridGap: "64px" }}
            >
              {techspireHeaderData[selectedYear].speakerLogos.map(
      ...
      {techspireHeaderData[selectedYear].industryLogos &&
        techspireHeaderData[selectedYear].industryLogos.length > 0 && (
          <>
            <Typography level="h3" py={2} textAlign={"center"}>
              Supported by
            </Typography>
            <Stack
              ...
              sx={{ gridGap: "32px" }}
            >
              {techspireHeaderData[selectedYear].industryLogos.map(
                (sponsor, idx) => renderLogoRows(idx, sponsor)
              )}
            </Stack>
          </>
      ...
      <PageSection title={`Speakers`}>
        <Grid container flexGrow={1} rowSpacing={3}>
          ...
          {(techspireSpeakerData[selectedYear] || []).length === 0 ? (
            <p style={{ margin: "auto" }}>Details coming soon</p>
          ) : null}
        </Grid>
      </PageSection>
    </PageSection>
